      ...
      return Promise.all(
        users.map(async (u) => {
            ...
            .map(
              (d) => `
                <tr>
                  ...
                  <td style="padding:4px 8px;">₹${(d.amount * USD_TO_INR_RATE).toFixed(2)}</td>
                </tr>
              `
            )
            .join("");
          ...
          const html = `
            ...
            <p>Hi ${u.name}, you have the following outstanding balances in INR:</p>
            <table cellspacing="0" cellpadding="0" border="1" style="border-collapse:collapse;">
              <thead>
                <tr><th>To</th><th>Amount (INR)</th></tr>
              </thead>
              <tbody>${rows}</tbody>
            </table>
          ...
          try {
            await convex.action(api.email.sendEmail, {
              to: u.email,
              subject: "You have pending payments in INR on Splitr",
              html,
